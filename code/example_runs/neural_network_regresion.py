...
def find_best_value():
    ...
    acc_lst[np.isnan(acc_lst)] = 5000
    index = np.argmin(acc_lst)
    print (value_lst[index])
    print ('mse', acc_lst[index])
...
def compare():
    ...
    for i in range(5):
        ...
        dnn = MLPRegressor(hidden_layer_sizes=100,
                            solver = 'sgd', alpha=0.005, learning_rate_init=0.008,batch_size = 32, max_iter=300)
        dnn.fit(reg.X_train, reg.f_train)
...
def convex_check():
    ...
    lr_lst= [0.000001,0.0001,0.01]
    for i in range(3):
        neunet = FFNN(reg.X_train, y_train, 2, 100, cost_func='SSR', hidden_func='leaky-relu' , output_func='linear',
        batch_size=32, eta=lr_lst[i], lam=0.05)
        loss = neunet.train(epoch = epoch_n)
        plt.plot(epoch, loss, label = lr_lst[i])
